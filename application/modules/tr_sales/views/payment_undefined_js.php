...
jQuery(document).ready(function() {
	...
	$('#datatable').dataTable({
		...
		"bFilter":true,
		"sServerMethod": "POST",
		"sAjaxSource": "<?=base_url()?>index.php/payment-undefined/DT",
	});
	$('#datatable2').dataTable({
		...
		"bFilter":true,
		"sServerMethod": "POST",
		"sAjaxSource": "<?=base_url()?>index.php/payment-defined/DT",
	});
	$('.chosen-select').val('').chosen().trigger('chosen:updated');
	// event
	$('#nounit').on('change', function() {
		var resno = $('#nounit').val();
		$('#idpay').val('');
		$('#kode_pay').val('');
		$('#nama').val('');
	...
	$('#btn-submit-pay').click(function() {
		...
		if(sErr==='') {
			$.post(
				'<?=base_url()?>index.php/payment-undefined/save',
				$('#form-payment').serialize(),
				function(respon) {
					// console.log(respon);
					if(respon!=='') {
						// if($('#'+$('#kode_pay').val()+$('#no_urut').val()).children().eq(4).text()==='') {
						// 	$('#'+$('#kode_pay').val()+$('#no_urut').val()).children().eq(4).text($('#tgl_bayar').val());
						// 	$('#'+$('#kode_pay').val()+$('#no_urut').val()).children().eq(5).text($('#rp').val());
						// 	$('#'+$('#kode_pay').val()+$('#no_urut').val()).children().eq(6).text(respon);
						// } else {
						// 	$('#'+$('#kode_pay').val()+$('#no_urut').val()).after(
						// 		'<tr id="'+$('#kode_pay').val()+$('#no_urut').val()+'"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td class="text-center">'+$('#tgl_bayar').val()+
						// 		'</td><td class="input-numeric text-right">'+$('#rp').val()+'</td><td class="text-center">'+respon+'</td><td>&nbsp;</td><td>&nbsp;</td></tr>'
						// 	);
						// }
						// $('.input-numeric').autoNumeric('init');
						// $('#sum-ri').autoNumeric('set',parseFloat($('#sum-ri').autoNumeric('get'))+parseFloat($('#rp').autoNumeric('get')));
						$('#idpay').val('');
						$('#kode_pay').val('');
						$('#nama').val('');
