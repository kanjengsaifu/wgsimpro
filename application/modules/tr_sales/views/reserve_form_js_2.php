...
jQuery(document).ready(function() {
	...
	$('#kode_pay').on('change', function() {
		if($('#no_unit').val()!=='') {
			var sPayMode = $(this).val(), 
				strPayMode = $('#kode_pay option[value="'+sPayMode+'"]').text(),
				sDiv = $('#pay-container').html("");
			var priceP = $('#pharga-'+sPayMode);
			if(priceP.length!==0) {
				$('#harga').val(priceP.autoNumeric('get'));
				$.post(
				...
				$.post(
					...
					function(respon) {
						...
						if(respon.status==='200') {
							alert(respon.msg);
							location.href="<?=base_url()?>index.php/sales-dashboard";
						} else {
							alert(respon.msg);
						}
					}
	...
	// custom
	<?php if(isset($data['no_unit'])) { ?>
	$('#eno_unit').attr('data-encunit','<?=$data['no_unit']?>');
	$('.row-unit').trigger('click');
	<?php } ?>
	// modal diskon
	$('#modal-diskon').modal({
		show: false,
