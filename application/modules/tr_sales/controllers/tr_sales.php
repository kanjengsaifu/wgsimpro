...
class Tr_sales extends App {
	...
	public function __construct() {
        ...
        $this->load->model('../../tr_accounting/models/tr_accounting_model');
...
//        print_r($this->tr_sales_model->get_grouped_unit());
	...
	public function form_undefined() {
		$data = array();
		$data['idmenu'] = 'penerimaan-undefined-form';
		$data['content'] = '../../tr_sales/views/undefined_form';
		$data['datacontent']['bank'] = $this->tr_sales_model->get_bank_ops();
		$data['js'] = '../../tr_sales/views/undefined_form_js';
		$this->buildView($data);
	}
	
	public function form_bank_in() {
		$data = array();
		$data['idmenu'] = 'bank-in';
		$data['content'] = '../../tr_sales/views/form_bank_in';
		$data['datacontent']['unit']	= $this->tr_sales_model->get_nasabah();
		$data['datacontent']['bank'] = $this->tr_sales_model->get_bank_ops();
		$data['js'] = '../../tr_sales/views/form_bank_in_js';
		$this->buildView($data);
	}

	public function form_bank_in_post() {
		$data = array();
		$data['idmenu'] = 'bank-post';
		$data['content'] = '../../tr_sales/views/form_bank_in_post';
		$data['js'] = '../../tr_sales/views/form_bank_in_post_js';
		$this->buildView($data);
	}
	
	public function form_bank_in_voucher() {
		$data = array();
		$data['idmenu'] = 'bank-print';
		$data['content'] = '../../tr_sales/views/form_bank_in_voucher';
		$data['js'] = '../../tr_sales/views/form_bank_in_voucher_js';
		$this->buildView($data);
	}
	...
	public function form_payment_undefined() {
		$data = array();
		$data['idmenu'] = 'penerimaan-undefined';
		$data['datacontent']['unit']	= $this->tr_sales_model->get_unit_undefiend();
		$data['content'] = '../../tr_sales/views/payment_undefined';
		$data['js'] = '../../tr_sales/views/payment_undefined_js';
		$this->buildView($data);
	}
	...
	public function form_change_pola() {
		$data = array();
		$data['idmenu'] = 'sales-change-pola';
		$data['content'] = '../../tr_sales/views/change_pola';
		$data['datacontent']['saleses'] = $this->tr_sales_model->get_saleses();
		$data['datacontent']['paymodes'] = $this->tr_sales_model->get_paymodes();
		$data['datacontent']['bankkpr'] = $this->tr_sales_model->get_bank_kpr();
		$data['js'] = '../../tr_sales/views/change_pola_js';
		$this->buildView($data);
	}
	...
	public function genDT_payment_undefined(){
		$this->datatables->select('tr_undefined.id as id,bk.nama as bank,bk.no_rekening as no_rekening,tr_undefined.nama as nama,rp,FORMAT(rp,2)as rp_tampil,DATE_FORMAT(tanggal,"%d/%m/%Y") as tanggal',FALSE)
			 ->unset_column('id')
			 ->unset_column('rp')
			 ->unset_column('nama')
			 ->from('tr_undefined')
			 ->join('mst_bank AS bk', 'bk.kode=tr_undefined.kode AND bk.kode_entity = "'.$this->session->userdata('kode_entity').'"', 'inner')
			 ->where('reserve_no = "" OR reserve_no IS NULL')
			 ->add_column('action','<a href="javascript:" class="row-data" data-id="$1" data-tanggal="$2" data-nominal="$3"><span class="glyphicons glyphicons-check"></span> View</a>', 
					'id,tanggal,rp');
		echo $this->datatables->generate();
	}

	public function genDT_payment_defined(){
		$this->datatables->select('tr_undefined.id as id,bk.nama as bank,bk.no_rekening as no_rekening,ns.kode as kode,ns.nama as nama,FORMAT(rp,2)as rp,DATE_FORMAT(tanggal,"%d/%m/%Y") as tanggal,tr_undefined.reserve_no',FALSE)
			 ->unset_column('id')
			 ->from('tr_undefined')
			 ->join('mst_bank AS bk', 'bk.kode=tr_undefined.kode AND bk.kode_entity = "'.$this->session->userdata('kode_entity').'"', 'inner')
			 ->join('tr_payment AS pay', 'pay.reserve_no=tr_undefined.reserve_no', 'inner')
			 ->join('mst_nasabah AS ns', 'ns.kode=pay.kode_nasabah', 'inner')
			 ->where('tr_undefined.reserve_no <> "" OR tr_undefined.reserve_no IS NOT NULL');
		echo $this->datatables->generate();
	}
	
	public function genDT_bank_in(){
		$this->datatables->select('cb.id as id,cb.kode_bank,cb.kode_nasabah,cb.no_voucher,DATE_FORMAT(cb.tanggal,"%d/%m/%Y") as tanggal,bk.nama as nama_bank,ns.nama as nama,cb.keterangan, FORMAT(cb.rp,2) as rp,cb.jenis_transaksi',FALSE)
			 ->unset_column('id')
			 ->unset_column('cb.kode_bank')
			 ->unset_column('cb.kode_nasabah')
			 ->unset_column('cb.jenis_transaksi')
			 ->from('tr_cashbank cb')
			 ->join('mst_bank AS bk', 'bk.kode=cb.kode_bank AND bk.kode_entity = "'.$this->session->userdata('kode_entity').'"', 'inner')
			 ->join('mst_nasabah AS ns', 'ns.kode=cb.kode_nasabah', 'inner')
			 ->where('status','f')
			 ->add_column('action','<a href="javascript:" class="row-data text-system" data-id="$1" data-tanggal="$2" data-kdbank="$3" data-kdns="$4" data-rp="$5" data-ket="$6" data-tr="$7"><span class="glyphicons glyphicons-pencil"></span>Edit</a>&nbsp;&nbsp;|&nbsp;&nbsp;'.
			 					   '<a href="javascript:" class="row-delete text-danger" data-id="$1" ><span class="glyphicons glyphicons-remove"></span>Delete</a>', 
					'id,tanggal,cb.kode_bank,cb.kode_nasabah,rp,keterangan,cb.jenis_transaksi');
		echo $this->datatables->generate();
	}

	public function genDT_bank_in_post(){
		$this->datatables->select('cb.id as id,cb.no_voucher,DATE_FORMAT(cb.tanggal,"%d/%m/%Y") as tanggal,bk.nama as nama_bank,ns.nama as nama,cb.keterangan, FORMAT(cb.rp,2) as rp',FALSE)
			 ->unset_column('id')
			 ->from('tr_cashbank cb')
			 ->join('mst_bank AS bk', 'bk.kode=cb.kode_bank AND bk.kode_entity = "'.$this->session->userdata('kode_entity').'"', 'inner')
			 ->join('mst_nasabah AS ns', 'ns.kode=cb.kode_nasabah', 'inner')
			 ->where('status','f')
			 ->add_column('action','<a href="javascript:" class="row-data text-system" data-id="$1" ><span class="glyphicons glyphicons-ok"></span>Post</a>', 
					'id');
		echo $this->datatables->generate();
	}
	public function genDT_bank_in_voucher(){
		$this->datatables->select('cb.id as id,cb.no_voucher,DATE_FORMAT(cb.tanggal,"%d/%m/%Y") as tanggal,bk.nama as nama_bank,ns.nama as nama,cb.keterangan, FORMAT(cb.rp,2) as rp',FALSE)
			 ->unset_column('id')
			 ->from('tr_cashbank cb')
			 ->join('mst_bank AS bk', 'bk.kode=cb.kode_bank AND bk.kode_entity = "'.$this->session->userdata('kode_entity').'"', 'inner')
			 ->join('mst_nasabah AS ns', 'ns.kode=cb.kode_nasabah', 'inner')
			 ->where('status','t')
			 ->add_column('action','<a href="javascript:" class="row-data text-system" data-id="$1" ><span class="glyphicons glyphicons-ok"></span>Cetak</a>', 
					'id');
		echo $this->datatables->generate();
	}
    ...
    public function get_change_payment($res_no, $cara_bayar, $kode_pay, $plan = 'PLAN1') {
        $this->output->set_content_type('application/json');
		echo json_encode($this->tr_sales_model->get_change_payment($res_no, $cara_bayar, $kode_pay, $plan = 'PLAN1'));
    }
	...
	public function save_confirm_payment() {
       ...
       /* $data = array(
					'is_app'			=>'a',
					'doc_no'			=>$this->tr_accounting_model->get_jm_no(),
					'date_input'		=>date('Y-m-d'),
					'rfp_total'			=>str_replace(',','',$this->input->post('rp')),
					'rp'				=>str_replace(',','',$this->input->post('rp')),
					'pay_to'			=>$this->input->post('nama'),
					'remarks'			=>$this->input->post('reserve_no'),
					'kode_entity'		=>$this->session->userdata('kode_entity')
				);
		$h = $this->tr_accounting_model->save_rfp_batch($data);
        
        $data = array(
					'idrfp'				=>$h,
					'coa'				=>$this->input->post('coa'),
					'rp'				=>str_replace(',','',$this->input->post('rp_detail')),
        ...
        $data = array(
					'is_app'			=>'r',
					'doc_no'			=>$this->tr_accounting_model->get_bm_no(),
					'date_input'		=>date('Y-m-d'),
					'code_doc'			=>'BM',
					'rfp_total'			=>str_replace(',','',$this->input->post('rp')),
					'rp'				=>str_replace(',','',$this->input->post('rp')),
        ...
        $h = $this->tr_accounting_model->save_ledger($data);
        $data = array(
					'idledger'			=>$h,
					'coa'				=>'11121',
					'kode_bank'			=>$this->input->post('kode'),
					'rp'				=>str_replace(',','',$this->input->post('rp')),
					'dk'				=>'D',
					'keterangan'		=>$this->input->post('reserve_no')
				);
		$h = $this->tr_accounting_model->save_ledger_detail($data);
		echo $no_kwitansi;
	}
	public function save_data_payment_undefined(){
		$data = array(
					'kode'			=>$this->input->post('kode'),
					/*'nama'			=>$this->input->post('nama'),
					'no_rekening'	=>$this->input->post('no_rekening'),*/
					'tanggal'		=>$this->dateutils->dateStr_to_mysql($this->input->post('tanggal')),
					'rp'			=>str_replace(',','',$this->input->post('rp'))
				);
		$h = $this->tr_sales_model->save_undefined($data);
		echo $data['kode'];
	}
	public function save_confirm_payment_undefined() {
		$stgl = $this->dateutils->dateStr_to_mysql($this->input->post('tgl_bayar'));
		// list($t,$b,$h) = explode('-',$stgl);
		$no_kwitansi = '0';//sprintf('KWT-%s%s%s%06d',substr($t,2),$b,$h,$this->input->post('idpay'));
		$data = array(
		...
		elseif($data['kode_pay']==='TJ')
			...
			$status = 'SALES';
		$dt= array(
				'no_entity'=>$this->session->userdata('kode_entity'),
				'reserve_no'=>$this->input->post('reserve_no'),
				'tanggal_update'=>date('Y-m-d')
			 );
		$this->tr_sales_model->save_payment_undefined($dt,$this->input->post('idundefined'));
		$this->tr_sales_model->save_confirm_payment($data, $status);
		echo $no_kwitansi;
	}

	public function save_data_bank_in(){
		$data = array(
						'kode_bank'=> $this->input->post('kode_bank'),
		...
		echo json_encode(array('id'=>$this->tr_sales_model->save_bank_in($data)));
	}
	
	public function update_data_bank_in(){
		$data = array(
						'kode_bank'=> $this->input->post('kode_bank'),
						'kode_nasabah'=> $this->input->post('kode_nasabah'),
						'rp'=> str_replace(',','',$this->input->post('rp')),
		...
		echo json_encode(array('id'=>$this->tr_sales_model->update_bank_in($data,$this->input->post('idbank'))));
	}
	
	public function save_data_bank_in_post($id){
		$id = $this->tr_sales_model->save_kode_bank_in_post($id);
