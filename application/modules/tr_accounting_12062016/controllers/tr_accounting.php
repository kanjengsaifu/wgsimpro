...
class Tr_accounting extends App {
    ...
    */
    ...
    function cek_double_entry(){
        ...
        $res = $this->tr_accounting_model->_doubleEntryNoBukChecking($data);

        if($res==true) {
            $out = array(
                'response'=>'1',
                'msg'=>'aya_euy',
                'pesan'=>'Nomor Bukti yang hendak anda simpan sudah terpakai. Harap segera ganti'//'Awas kang, nomor bukti eta aya didatabase na! Ngopi2 heula atuh!'
            );
        } else {
            $out = array(
                'response'=>'0',
                'msg'=>'teu_aya',
                'pesan'=>'Mangga, dilajutkeun... dahar heula aing ah!'
            );
        }
	...
	function save_jurnal()
	{
        ...
        $this->db->query("CALL updateCoaCommon('".$kd_spk."')");
		//var_dump($res);die;
        //if($this->input->post('is_mode')=='edit')
		/*if($res==true) {
			$out = array(
    ...
    private $before = 0;
    private $after = 0;
    function autoupdate_coacommon(){
        $kode = $this->session->userdata('kode_entity'); //$this->input->post('kode_coa');
        

        $q1= $this->db->query("select count(*) as jum from tr_coa_common where kode_spk='$kode'");
        foreach ($q1->result() as $row)
        {
           $before = $row->jum;
        }
        $res = $this->db->query("CALL updateCoaCommon('".$kode."')");
        $q2 = $this->db->query("select count(*) as jum from tr_coa_common where kode_spk='$kode'");
    ...
    */
    ...
    function doubleEntryNoBukChecking(){
        $nobuk = $this->input->post();
        return $this->tr_accounting_model->_doubleEntryNoBukChecking($nobuk);
    }

   function get_listnobuk($jenis,$periode){
        $res = $this->tr_accounting_model->_get_listnobuk($jenis,$periode);
