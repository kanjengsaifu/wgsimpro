...
.balance{
	...
	if ($num > 0){	
		...
		<tr id="tr_<?=$row->tid; ?>">
			...
			<td align='left'> <?=($row->kode_coa==''?'<input type="hidden" id="kdcoa_false[]" class="kdcoa_false" value="0">':$row->kode_coa); ?> </td>
            ...
            <td align='left'> <?=($row->kode_spk==$kode_spk ?'<input type="hidden" id="spk_false[]" class="spk_false" value="0">'.$row->kode_spk:'<input type="hidden" id="spk_false[]" class="spk_false" value="1"><span class="trx_exist">'.$row->kode_spk.'</span>'); ?> </td>
    ...
    <script type="text/javascript">
    	jQuery(document).ready(function() {
		    ...
		    $('.spk_false').each(function (index, element) {
		        spk = spk + parseFloat($(element).val());
		    });
    		...
    		total = t_bank+t_mnas+t_sbd+t_tahap+t_pajak+spk+balance;
    		//alert(total);
    		if(total>=1) {
    			alert('PERHATIAN !!\n\nKolom yang Tidak Dapat Diproses adalah \n - Kode Nasabah:'+t_mnas+' baris error/wajib isi, \n '
    									+'- Kode Tahap: '+t_tahap+' baris error/wajib isi \n '
    									+'- Faktur Pajak: '+t_pajak+' baris error/wajib isi \n '
    									+'- Sumberdaya: '+t_sbd+' baris error/wajib isi \n '
    									+'- Kode SPK: '+spk+' baris error/tidak sesuai\n '
