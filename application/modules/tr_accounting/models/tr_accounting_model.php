...
class Tr_accounting_model extends CI_Model {
    ...
    function getkode_wil($spk){
        $spk = $this->session->userdata('kode_entity');
        $this->db->select('kode_trjurnal');
        $this->db->from('mst_entity');
        $this->db->where(array('kode'=>$spk));
...
//var_dump($res);die;
    ...
    function _doubleEntryNoBukChecking($nobuk){
        $this->db->select('no_bukti, posted_by')
            ->from('tr_accounting')
            ->where(array('no_bukti'=>trim($nobuk['nobukti'])));
        $q = $this->db->get();
        //return $q->row_array();
        $no_bukti = '';
        //echo $this->db->last_query();
        foreach($q->result() as $row => $v){
            $no_bukti = $v['no_bukti'];
            $posted_by = $v['posted_by'];
        }
        if ($this->db->affected_rows() > 0) {
            //return true;
            $data = array('msg'=>'aya_euy','response'=>'aya_euy','action'=>'insert','pesan'=>'nomor bukti '.$no_bukti.' inserted!','res'=>true);
        } else {
            //return false;
            $data = array('msg'=>'can_aya','response'=>'can_aya','action'=>'insert','pesan'=>'nomor bukti '.$no_bukti.' inserted!','res'=>false);
        }  
        // var_dump($data);die;
        return $q->result();
    }
    ...
    function _cek_double_entry($nobuk){
        $this->db->select('no_bukti');
        $this->db->where(array('no_bukti'=>$nobuk));
        $this->db->from('tr_accounting');
        return $this->db->get();
        //return $this->db->query("SELECT no_bukti FROM tr_accounting WHERE no_bukti='".$nobuk."'");
    }
