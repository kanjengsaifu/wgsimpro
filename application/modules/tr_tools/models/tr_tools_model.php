...
class Tr_tools_model extends CI_Model {
    ...
    function _getListNoBuk($jenis='M',$periode){
        ...
        $sql = "SELECT DISTINCT no_bukti
                FROM tr_accounting
                WHERE 
                    MONTH(tanggal) = '$periode[1]' AND 
                    YEAR(tanggal) = '$periode[0]'  
                    $s_nobuk
                    AND kode_spk='$spk'
                GROUP BY no_bukti
        ...
        foreach($result as $row => $v){
            list($nobuk[], $bln, $jnis, $thn) = explode('/', $v['no_bukti']);
        ...
        for($i=0;$i<1000;$i++){
            ...
            if($nomor[0]===$nobuk[$i]){
               ...
               $res[] = array('nomor'=>$nomor[0],'status'=>'Terpakai');
            }else{
                //echo ($i+1).',  '.$nomor[0].'<br>';
                $res[] = array('nomor'=>$nomor[0],'status'=>'Kosong');
            }
        }
        //var_dump($res);die();
        /*    
