...
jQuery(document).ready(function() {
    ...
    */
	$("#kd_coa").autocomplete({
        ...
        function(req, add){
            $.ajax({
                ...
                url: "<?=base_url()?>index.php/jurnal/lookupCommon/"+$('#kd_jenis').val(),
                dataType: 'json',
                type: 'POST',
                data: req,
                success:
    ...
    $("#kd_coa").keydown(function(event) {
        ...
        if ( event.keyCode == 27 || event.keyCode == 46 || event.keyCode == 63 || event.keyCode == 8 || event.keyCode == 13 ) { //titik  atau backspace
            if(event.keyCode == 27){
    ...
    $('#kd_jenis').change(function() {
        ...
        if(kode == "") {
            ...
            $.ajax({
                ...
                success: function (response, textStatus) {
                    if(response.kbm=='M'){
                        $('#last_nobuk1').html('LAST_MEMORIAL: '+response.b_memo);
                        $('#last_nobuk2').html('');
                    }else{
                    ...
                    console.log(response.b_kas+' :: '+response.b_bank+' :: '+response.b_memo)
                },
                error: function (xhr, textStatus, errorThrown) {
                    alert("Error: " + (errorThrown ? errorThrown : xhr.status));
                }
