				...
				<div class="panel-body" style="overflow-x: scroll">
					...
					<div class="row">
						<div class="col-md-12 text-center">
							<h4 class="mbn">LAPORAN PENAGIHAN <?=isset($data['addtitle'])?$data['addtitle']:''?></h4>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12 text-center">
					...
					<table id="example" class="table table-bordered mbn">
						<thead>
							<tr class="bg-primary light">
								...
								<th rowspan="2" class="text-center" style="vertical-align:middle">Tower / Cluster</th>
								...
								<?=isset($data['addtitle'])?'':'<th rowspan="2" class="text-center" style="vertical-align:middle">Cara Bayar</th>'?>
								
								<th colspan="3" class="text-center">RENCANA PEMBAYARAN</th>
								<th colspan="3" class="text-center">REALISASI PEMBAYARAN</th>
								<th colspan="3" class="text-center">KEKURANGAN PEMBAYARAN</th>
								<!--<th colspan="2" class="text-center">JATUH TEMPO</th>-->
							</tr> 
							<tr class="bg-primary light">
								<!-- RA -->
								<th class="text-center">s/d Minggu Lalu</th>
						...
						<?php 
						$no = 1;
						$sumBruto = $sumRaLalu = $sumRaIni = $sumRiIni = $sumKRLalu = $sumKRIni = $sumRaDepan = $sumSDLalu = $sumTotal = 0;
						foreach($data['datatable'] as $k => $v) { 
							$ra_lalu = ($v['ra_um_lalu']+$v['ra_ag_lalu'] > $v['ri_mggulalutotal'])?($v['ra_um_lalu']+$v['ra_ag_lalu'])-$v['ri_mggulalutotal']:($v['ra_um_lalu']+$v['ra_ag_lalu']);
							$ra_ini = $v['ra_um_ini']+$v['ra_ag_ini'];
							$ri_ini = $v['ri_um_ini']+$v['ri_ag_ini'];
							$ri_total =  $v['ri_total'];
							//if($ra_lalu < 0)
							//	continue;
							// $ra_depan = $v['ra_um_depan'] + $v['ra_ag_depan'];
							$sisa_ri = $ri_total-$ri_ini;
							$kr_lalu = 0;
							if($ra_lalu>0) {
							...
							if($v['cara_bayar'] === 'KPR / KPA'){
								$kr_lalu 	= $v['krg_bayar_total'];
								$kr_ini 	= $v['krg_bayarini_total'];
							}
						...
						?>
							<tr>
								...
								<?=isset($data['addtitle'])?'':'<td class="text-center">'.$v['cara_bayar'].'</td>'?>
								...
								<td class="text-right"><?=number_format($ri_total-$ri_ini,2)?></td>
								<td class="text-right"><?=number_format($ri_ini,2)?></td>
								<td class="text-right"><?=number_format($ri_total,2)?></td>
						...
						<?php 
							...
							<tr class="bg-primary light">
								...
								<td class="text-right"><b><?=@number_format($sumSDLalu,2)?></b></td>
								<td class="text-right"><b><?=@number_format($sumRiIni,2)?></b></td>
								<td class="text-right"><b><?=@number_format($sumTotal,2)?></b></td>
